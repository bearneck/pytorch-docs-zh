...
上一节讨论了函数特定的参数。函数特定参数只能改变填充张量的值，有时也会改变张量的大小。它们从不改变所创建张量的数据类型（例如 [float32]{.title-ref} 或 [int64]{.title-ref}）或存储位置（CPU 或 GPU 内存）等属性。这些属性的指定留给了每个工厂函数的最后一个参数：一个 [TensorOptions]{.title-ref} 对象，将在下文讨论。

[TensorOptions]{.title-ref} 是一个封装了张量构造轴的类。所谓\*构造轴\*，是指张量在构造前可以配置（有时在构造后可以更改）的特定属性。这些构造轴包括：

- [dtype]{.title-ref}（以前称为"标量类型"），控制存储在张量中元素的数据类型，
- [layout]{.title-ref}，可以是 strided（密集）或 sparse，
- [device]{.title-ref}，表示存储张量的计算设备（如 CPU 或 CUDA GPU），
- [requires_grad]{.title-ref} 布尔值，用于启用或禁用张量的梯度记录。

如果您熟悉 Python 中的 PyTorch，这些轴听起来会非常熟悉。目前这些轴允许的值为：

- 对于 [dtype]{.title-ref}：\`kUInt8\`、\`kInt8\`、\`kInt16\`、\`kInt32\`、\`kInt64\`、\`kFloat32\` 和 [kFloat64]{.title-ref}，
- 对于 [layout]{.title-ref}：\`kStrided\` 和 [kSparse]{.title-ref}，
- 对于 [device]{.title-ref}：\`kCPU\` 或 [kCUDA]{.title-ref}（后者接受可选的设备索引），
- 对于 [requires_grad]{.title-ref}：\`true\` 或 [false]{.title-ref}。
...
存在 dtype 的"Rust 风格"简写，例如用 [kF32]{.title-ref} 代替 [kFloat32]{.title-ref}。完整列表请参见 [此处](https://github.com/pytorch/pytorch/blob/main/torch/csrc/api/include/torch/types.h)。


[TensorOptions]{.title-ref} 的实例为每个轴存储一个具体值。以下是一个创建 [TensorOptions]{.title-ref} 对象的示例，该对象表示一个 64 位浮点、strided 布局、需要梯度、存储在 CUDA 设备 1 上的张量：
...
请注意，我们如何使用 [TensorOptions]{.title-ref} 的"构建器"风格方法逐步构造对象。如果我们将此对象作为最后一个参数传递给工厂函数，新创建的张量将具有这些属性：
...
- dtype 默认为 [kFloat32]{.title-ref}，
- layout 默认为 [kStrided]{.title-ref}，
- device 默认为 [kCPU]{.title-ref}，
- [requires_grad]{.title-ref} 默认为 [false]{.title-ref}。

这意味着在构造 [TensorOptions]{.title-ref} 对象时，您省略的任何轴都将采用其默认值。例如，这是我们之前的 [TensorOptions]{.title-ref} 对象，但 [dtype]{.title-ref} 和 [layout]{.title-ref} 使用了默认值：

``` cpp
auto options = torch::TensorOptions().device(torch::kCUDA, 1).requires_grad(true);
```

实际上，我们甚至可以省略所有轴，得到一个完全使用默认值的 [TensorOptions]{.title-ref} 对象：

``` cpp
auto options = torch::TensorOptions(); // 或 `torch::TensorOptions options;`
```

一个很好的结果是，我们刚才讨论了很多的 [TensorOptions]{.title-ref} 对象可以从任何张量工厂调用中完全省略：
...
但便利性更进一步：在目前介绍的 API 中，您可能已经注意到初始的 [torch::TensorOptions()]{.title-ref} 写起来相当冗长。好消息是，对于每个构造轴（dtype、layout、device 和 [requires_grad]{.title-ref}），在 [torch::]{.title-ref} 命名空间中都有一个\*自由函数\*，您可以传递该轴的值。每个函数返回一个预先配置了该轴的 [TensorOptions]{.title-ref} 对象，但允许通过上面展示的构建器风格方法进行进一步修改。例如，
...
这为我们节省了不少打字量。这意味着在实践中，您几乎不需要写出 [torch::TensorOptions]{.title-ref}。而是使用 [torch::dtype()]{.title-ref}、\`torch::device()\`、\`torch::layout()\` 和 [torch::requires_grad()]{.title-ref} 函数。

最后一点便利是，\`TensorOptions\` 可以从单个值隐式构造。这意味着，每当函数有一个类型为 [TensorOptions]{.title-ref} 的参数（所有工厂函数都是如此），我们可以直接传递像 [torch::kFloat32]{.title-ref} 或 [torch::kStrided]{.title-ref} 这样的值来代替完整的对象。因此，当我们只想改变单个轴相对于其默认值时，我们可以只传递该值。这样，
...
当然，使用这种简写语法无法进一步修改 [TensorOptions]{.title-ref} 实例的其他属性，但如果我们只需要更改一个属性，这已经相当实用了。

总而言之，我们现在可以比较 [TensorOptions]{.title-ref} 的默认值，以及使用自由函数创建 [TensorOptions]{.title-ref} 的简写 API，如何使 C++ 中的张量创建与 Python 中一样方便。比较以下 Python 调用：
...
正如我们可以使用 [TensorOptions]{.title-ref} 来配置应如何创建新张量一样，我们也可以使用 [TensorOptions]{.title-ref} 将张量从一组属性转换为另一组属性。这种转换通常会创建一个新张量，并且不是原地进行的。例如，如果我们有一个通过以下方式创建的 [source_tensor]{.title-ref}：

``` cpp
torch::Tensor source_tensor = torch::randn({2, 3}, torch::kInt64);
```

我们可以将其从 [int64]{.title-ref} 转换为 [float32]{.title-ref}：
...
转换的结果 [float_tensor]{.title-ref} 是一个指向新内存的新张量，与源张量 [source_tensor]{.title-ref} 无关。
...
如果您有多个可用的 CUDA 设备，上述代码会将张量复制到\*默认\*的 CUDA 设备，您可以使用 [torch::DeviceGuard]{.title-ref} 进行配置。如果没有设置 [DeviceGuard]{.title-ref}，这将是 GPU 1。如果您想指定不同的 GPU 索引，可以将其传递给 [Device]{.title-ref} 构造函数：

``` cpp
torch::Tensor gpu_two_tensor = float_tensor.to(torch::Device(torch::kCUDA, 1));
```

在 CPU 到 GPU 复制及反向复制的情况下，我们还可以通过将 [/\*non_blocking=\*/false]{.title-ref} 作为最后一个参数传递给 [to()]{.title-ref} 来配置内存复制为\*异步\*：
