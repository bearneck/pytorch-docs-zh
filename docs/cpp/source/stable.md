...
这是 `TORCH_LIBRARY` 的稳定 ABI 等效版本。 使用此宏定义将在跨 PyTorch 版本时保持二进制兼容性的算子模式。每个命名空间只能存在一个 `STABLE_TORCH_LIBRARY` 块；对于来自不同翻译单元的同一命名空间中的额外定义，请使用 `STABLE_TORCH_LIBRARY_FRAGMENT`。
