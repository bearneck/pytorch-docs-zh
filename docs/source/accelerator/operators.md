...
{ref}`上面提到的算子 <operator-set>` 有一个共同特点：它们都是具有已定义 `命名空间` 和 `Schemas` 的内置 PyTorch 算子，并且这些算子的内置加速器（`CPU`、`CUDA` 等）已经实现。我们接下来要做的就是为新的加速器实现这些算子。
...
遵循 {ref}`步骤 1<step-one>`，我们可以完成除 `fallback` 外所有算子的开发和注册。接下来，为了支持运算相关的算子（例如数学运算、卷积运算等），我们需要实现回退语义的注册。这是 PyTorch 框架提供的内置能力，可以将新加速器不支持的部分操作回退到 CPU 上执行。对于开发中的新后端，这是一种以性能为代价保证功能的极其有效的方式。
...
PyTorch 还为内置算子提供了另一种方法：`STUB`。这种方法本质上基于 {ref}`步骤 1<step-one>` 的方法，但增加了二次调度的能力（例如，基于 CPU 特性进行调度）。
...
2. **注册操作符**
    ...
    使用 `TORCH_LIBRARY_IMPL` 在 `PrivateUse1` 中为 `custom_abs` 操作符注册 `wrapper_custom_abs` 实现。因为 `Autograd` 在 PyTorch 中始终启用，即使仅需要前向计算，PyTorch 默认也会查找并执行相应的反向实现（在反向实现中会回退）。幸运的是，PyTorch 也为 PrivateUse1 实现了一个通用的 `Autograd Fallback`，如果只涉及前向计算，它相当于一个回退操作，选择下一个 DispatchKey 进行计算；如果涉及反向计算，则会抛出错误。
