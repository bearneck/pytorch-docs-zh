...
通信钩子提供了一种灵活的方式来 allreduce 梯度。因此，它主要在 allreduce 之前操作每个副本上的梯度，这些梯度被分桶处理以增加通信与计算之间的重叠。具体来说，{class}`torch.distributed.GradBucket` 表示一个待 allreduce 的梯度张量桶。
...
默认通信钩子是简单的**无状态**钩子，因此 `register_comm_hook` 中的输入状态要么是一个进程组，要么是 `None`。输入 `bucket` 是一个 {class}`torch.distributed.GradBucket` 对象。
...
此外，还提供了一个通信钩子包装器，以支持将 {meth}`~fp16_compress_hook` 或 {meth}`~bf16_compress_hook` 作为包装器，这可以与其他通信钩子结合使用。
...
{class}`PowerSGDState` 已经实现了 `__setstate__` 和 `__getstate__` 方法，可以作为参考。
