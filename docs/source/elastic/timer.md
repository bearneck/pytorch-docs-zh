...
要编写自己的计时器服务器和客户端，请分别继承 `torch.distributed.elastic.timer.TimerServer`（服务器）和 `torch.distributed.elastic.timer.TimerClient`（客户端）。 `TimerRequest` 对象用于在服务器和客户端之间传递消息。
