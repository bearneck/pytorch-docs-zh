...
但是，如果你想做类似计算模型参数雅可比矩阵的操作，那么需要一种方法来构造一个以参数为函数输入的函数。这就是 {func}`functional_call` 的用途：它接受一个 nn.Module、变换后的 `parameters` 以及 Module 前向传播的输入。它返回使用替换后的参数运行 Module 前向传播的值。
