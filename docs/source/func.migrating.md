...
| ----------------------------------- | ---------------------------------------------- |
| functorch.vmap                      | {func}`torch.vmap` 或 {func}`torch.func.vmap`              |
...
此外，如果您正在使用 torch.autograd.functional API，请尝试使用 {mod}`torch.func` 的等效功能。{mod}`torch.func` 函数变换在许多情况下更具可组合性和更高性能。

| torch.autograd.functional API               | torch.func API (自 PyTorch 2.0 起)                |
| ------------------------------------------- | ---------------------------------------------- |
| {func}`torch.autograd.functional.vjp`             | {func}`torch.func.grad` 或 {func}`torch.func.vjp`           |
| {func}`torch.autograd.functional.jvp`             | {func}`torch.func.jvp`                                |
| {func}`torch.autograd.functional.jacobian`        | {func}`torch.func.jacrev` 或 {func}`torch.func.jacfwd`      |
...
{func}`torch.func.functional_call` 是 [functorch.make_functional](https://pytorch.org/functorch/1.13/generated/functorch.make_functional.html#functorch.make_functional) 和 [functorch.make_functional_with_buffers](https://pytorch.org/functorch/1.13/generated/functorch.make_functional_with_buffers.html#functorch.make_functional_with_buffers) 的替代品。然而，它并非直接替代品。

如果您时间紧迫，可以使用 [此 gist 中的辅助函数](https://gist.github.com/zou3519/7769506acc899d83ef1464e28f22e6cf) 来模拟 functorch.make_functional 和 functorch.make_functional_with_buffers 的行为。我们建议直接使用 {func}`torch.func.functional_call`，因为它是一个更明确和灵活的 API。

具体来说，functorch.make_functional 返回一个功能模块和参数。该功能模块接受模型的参数和输入作为参数。{func}`torch.func.functional_call` 允许使用新的参数、缓冲区和输入调用现有模块的前向传播。

以下是一个使用 functorch 与 {mod}`torch.func` 计算模型参数梯度的示例：
...
请使用 {func}`torch.func.stack_module_state` 替代
...
{func}`torch.func.stack_module_state` 返回两个字典，一个包含堆叠的参数，另一个包含堆叠的缓冲区，然后可以与 {func}`torch.vmap` 和 {func}`torch.func.functional_call`
一起用于集成学习。
