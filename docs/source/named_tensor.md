...
关于张量名称的限制，请参阅 `~Tensor.names`。

使用 `~Tensor.names` 访问张量的维度名称，并使用 `~Tensor.rename` 重命名维度。
...
命名张量可以与未命名张量共存；命名张量是 `torch.Tensor` 的实例。未命名张量的维度名称为 ``None``。命名张量并不要求所有维度都被命名。
...
```{note}
在实践中，使用命名张量时，应避免存在未命名维度，因为它们的处理可能很复杂。建议使用 `~Tensor.refine_names` 将所有未命名维度提升为命名维度。
```
...
有关名称推断规则的完整列表，请参阅 `name_inference_reference-doc`。以下是两个常见操作，可能有助于理解：

- 二元算术运算：`unifies_names_from_inputs-doc`
- 矩阵乘法运算：`contracts_away_dims-doc`

## 按名称显式对齐

使用 `~Tensor.align_as` 或 `~Tensor.align_to` 按名称将张量维度对齐到指定的顺序。这对于执行"按名称广播"非常有用。
...
使用 `~Tensor.align_to` 可以置换大量维度，而无需像 `~Tensor.permute` 那样提及所有维度。
...
分别使用 `~Tensor.flatten` 和 `~Tensor.unflatten` 来展平和展开维度。这些方法比 `~Tensor.view` 和 `~Tensor.reshape` 更冗长，但对于阅读代码的人来说具有更多的语义含义。
...
有关支持的 torch 和张量操作的完整列表，请参阅 `name_inference_reference-doc`。以下操作目前尚未支持（链接中未涵盖）：
...
自动求导已支持，请参阅 `named_tensors_autograd-doc`。
...
在本节中，请查阅命名张量特定 API 的文档。有关名称如何通过其他 PyTorch 运算符传播的完整参考，请参阅 `name_inference_reference-doc`。
