...
在嵌套张量上调用操作的 API 与常规 ``torch.Tensor`` 并无不同，允许与现有模型无缝集成，主要区别在于 输入的构造 。

由于这是一个原型功能，支持的操作  集合有限，但正在不断增长。我们欢迎问题报告、功能请求和贡献。有关贡献的更多信息，请参阅 [此 Readme](https://github.com/pytorch/pytorch/blob/main/aten/src/ATen/native/nested/README.md)。
...
在上面的例子中，尽管两个 NJT 的概念形状相同，但它们没有引用同一个 ``offsets`` 张量，因此它们的形状不同，并且不兼容。我们认识到这种行为不够直观，正在努力为嵌套张量的 Beta 版本放宽此限制。有关解决方法，请参阅本文档的 故障排除  部分。
