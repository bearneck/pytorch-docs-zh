...
使用 `torch.nn.utils.parameterize.register_parametrization`{.interpreted-text role="func"} 中的新参数化功能实现的参数化。
...
量化指的是以低于浮点精度的位宽执行计算和存储张量的技术。PyTorch 支持每张量和每通道的非对称线性量化。要了解更多关于如何在 PyTorch 中使用量化函数的信息，请参阅 `quantization-doc`{.interpreted-text role="ref"} 文档。
