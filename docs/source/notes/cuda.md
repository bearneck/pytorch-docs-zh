 {.meta description="torch.cuda 使用指南，这是一个用于运行 CUDA 操作的 PyTorch 模块" keywords="内存管理, PYTORCH_ALLOC_CONF, 优化 PyTorch, CUDA"}


# CUDA 语义 {#cuda-semantics}

`torch.cuda`{.interpreted-text role="mod"} 用于设置和运行 CUDA 操作。它会跟踪当前选定的 GPU，并且默认情况下，您分配的所有 CUDA 张量都将在该设备上创建。选定的设备可以通过 `torch.cuda.device`{.interpreted-text role="any"} 上下文管理器来更改。

但是，一旦张量被分配，您就可以在其上执行操作，而不受选定设备的影响，并且结果将始终放置在与该张量相同的设备上。

默认情况下，不允许跨 GPU 操作，但 `~torch.Tensor.copy_`{.interpreted-text role="meth"} 和其他具有复制功能的方法（如 `~torch.Tensor.to`{.interpreted-text role="meth"} 和 `~torch.Tensor.cuda`{.interpreted-text role="meth"}）除外。除非您启用了点对点内存访问，否则任何尝试在不同设备上的张量上启动操作的尝试都会引发错误。
...
## Ampere（及更高版本）设备上的 TensorFloat-32 (TF32) {#tf32_on_ampere}
...
对于 [cuda/cudnn]{.title-ref}，fp32_precision 可以设置为 [ieee]{.title-ref} 或 [tf32]{.title-ref}。 [ieee]{.title-ref} fp32_precision 表示我们将使用 [FP32]{.title-ref} 作为内部计算精度。 [tf32]{.title-ref} fp32_precision 表示我们将允许使用 [TF32]{.title-ref} 作为内部计算精度。

如果 fp32_precision 设置为 [ieee]{.title-ref}，我们可以为特定操作符覆盖通用设置。
...
如果 fp32_precision 设置为 [ieee]{.title-ref}，我们也可以为特定后端覆盖通用设置。
...
对于上述两种情况，\`torch.backends.cudnn.conv.fp32_precision\` 和 [torch.backends.cudnn.rnn.fp32_precision]{.title-ref} 都被覆盖为 [ieee]{.title-ref}。
...
以下使用 [allow_tf32]{.title-ref} 的旧设置将被弃用。我们建议使用上述新设置以获得更好的控制。并且我们不支持混合使用新旧设置。


从 PyTorch 1.7 开始，有一个名为 [allow_tf32]{.title-ref} 的新标志。该标志在 PyTorch 1.7 到 PyTorch 1.11 中默认为 True，在 PyTorch 1.12 及更高版本中默认为 False。 此标志控制 PyTorch 是否允许在内部使用 TensorFloat32 (TF32) 张量核心（自 Ampere 以来在 NVIDIA GPU 上可用）来计算 matmul（矩阵乘法和批量矩阵乘法）和卷积。

TF32 张量核心旨在通过将输入数据舍入为 10 位尾数，并以 FP32 精度累积结果，同时保持 FP32 动态范围，从而在 [torch.float32]{.title-ref} 张量上实现更好的 matmul 和卷积性能。
...
matmul 的精度也可以通过 `~torch.set_float32_matmul_precision`{.interpreted-text role="meth"} 更广泛地设置（不仅限于 CUDA）。 请注意，除了 matmul 和卷积本身之外，内部使用 matmul 或卷积的函数和 nn 模块也会受到影响。这些包括 [nn.Linear]{.title-ref}、\`nn.Conv\*\`、cdist、tensordot、affine grid 和 grid sample、adaptive log softmax、GRU 和 LSTM。
...
## FP16 GEMM 中的降低精度归约 {#fp16reducedprecision}

（不同于为那些 FP16 累加比 FP32 累加具有更高吞吐量的硬件设计的完整 FP16 累加，参见 `完整 FP16 累加<fp16accumulation>`{.interpreted-text role="ref"}）

fp16 GEMM 可能会使用一些中间降低精度归约（例如，使用 fp16 而非 fp32）来完成。这些选择性的精度降低可以在某些工作负载（特别是那些具有较大 [k]{.title-ref} 维度的负载）和 GPU 架构上实现更高的性能，但代价是数值精度和潜在的溢出风险。
...
## BF16 GEMM 中的降低精度归约 {#bf16reducedprecision}

对于 BFloat16 GEMM，存在一个类似的标志（如上所述）。 请注意，对于 BF16，此开关默认设置为 [True]{.title-ref}，如果您在工作负载中观察到数值不稳定性，您可能希望将其设置为 [False]{.title-ref}。
...
## FP16 GEMM 中的完整 FP16 累加 {#fp16accumulation}

某些 GPU 在 [完全]{#完全}\_ 使用 FP16 进行 FP16 GEMM 累加时性能会提高，但代价是数值精度和更大的溢出可能性。请注意，此设置仅对计算能力为 7.0（Volta）或更高版本的 GPU 有效。
...
异步计算的一个后果是，没有同步的时间测量是不准确的。为了获得精确的测量结果，应该在测量前调用 `torch.cuda.synchronize()`{.interpreted-text role="func"}，或者使用 `torch.cuda.Event`{.interpreted-text role="class"} 来记录时间，如下所示：:
...
作为例外，一些函数如 `~torch.Tensor.to`{.interpreted-text role="meth"} 和 `~torch.Tensor.copy_`{.interpreted-text role="meth"} 接受一个显式的 `non_blocking`{.interpreted-text role="attr"} 参数，允许调用者在不需要时绕过同步。另一个例外是 CUDA 流，如下所述。

### CUDA 流

一个 [CUDA 流](#cuda-流) 是属于特定设备的线性执行序列。通常您不需要显式创建它：默认情况下，每个设备使用自己的"默认"流。

每个流内部的操作按照它们创建的顺序串行执行，但来自不同流的操作可以以任何相对顺序并发执行，除非使用了显式的同步函数（如 `~torch.cuda.synchronize`{.interpreted-text role="meth"} 或 `~torch.cuda.Stream.wait_stream`{.interpreted-text role="meth"}）。例如，以下代码是不正确的：:
...
这里有两个新增部分。`torch.cuda.Stream.wait_stream`{.interpreted-text role="meth"} 调用确保在我们开始在侧流上运行 `sum(A)` 之前，`normal_()` 的执行已经完成。`torch.Tensor.record_stream`{.interpreted-text role="meth"}（详见其文档）确保在 `sum(A)` 完成之前不会释放 A 的内存。您也可以在稍后的时间点手动等待该流，使用 `torch.cuda.default_stream(cuda).wait_stream(s)`（注意立即等待是没有意义的，因为这会阻止该流与默认流上的其他工作并行执行。）关于何时使用哪一个的更多细节，请参阅 `torch.Tensor.record_stream`{.interpreted-text role="meth"} 的文档。
...
### 反向传播的流语义 {#bwd-cuda-stream-semantics}

每个反向 CUDA 操作在与对应前向操作相同的流上运行。如果前向传播在不同的流上并行运行独立操作，这有助于反向传播利用相同的并行性。

反向调用相对于周围操作的流语义与任何其他调用相同。反向传播会插入内部同步以确保这一点，即使反向操作在多个流上运行，如前一段所述。更具体地说，当调用 `autograd.backward<torch.autograd.backward>`{.interpreted-text role="func"}、`autograd.grad<torch.autograd.grad>`{.interpreted-text role="func"} 或 `tensor.backward<torch.Tensor.backward>`{.interpreted-text role="meth"}，并可选地提供 CUDA 张量作为初始梯度（例如，`autograd.backward(..., grad_tensors=initial_grads)<torch.autograd.backward>`{.interpreted-text role="func"}、`autograd.grad(..., grad_outputs=initial_grads)<torch.autograd.grad>`{.interpreted-text role="func"} 或 `tensor.backward(..., gradient=initial_grad)<torch.Tensor.backward>`{.interpreted-text role="meth"}）时，以下操作：
...
## 内存管理 {#cuda-memory-management}

PyTorch 使用缓存内存分配器来加速内存分配。这允许快速释放内存而无需设备同步。然而，分配器管理的未使用内存仍会在 `nvidia-smi` 中显示为已使用。你可以使用 `~torch.cuda.memory_allocated`{.interpreted-text role="meth"} 和 `~torch.cuda.max_memory_allocated`{.interpreted-text role="meth"} 来监控张量占用的内存，并使用 `~torch.cuda.memory_reserved`{.interpreted-text role="meth"} 和 `~torch.cuda.max_memory_reserved`{.interpreted-text role="meth"} 来监控缓存分配器管理的总内存量。调用 `~torch.cuda.empty_cache`{.interpreted-text role="meth"} 会释放 PyTorch 中所有\*\*未使用的\*\*缓存内存，以便其他 GPU 应用程序可以使用这些内存。但是，张量占用的 GPU 内存不会被释放，因此这不会增加 PyTorch 可用的 GPU 内存量。

为了更好地理解 CUDA 内存随时间的使用情况，`torch_cuda_memory`{.interpreted-text role="ref"} 描述了捕获和可视化内存使用跟踪的工具。

对于更高级的用户，我们通过 `~torch.cuda.memory_stats`{.interpreted-text role="meth"} 提供了更全面的内存基准测试。我们还提供了通过 `~torch.cuda.memory_snapshot`{.interpreted-text role="meth"} 捕获内存分配器状态完整快照的能力，这可以帮助你理解代码产生的底层分配模式。

### 使用 `PYTORCH_ALLOC_CONF` 优化内存使用 {#cuda-memory-envvars}
...
- `max_split_size_mb` 阻止原生分配器拆分大于此大小（以 MB 为单位）的块。这可以减少碎片，并可能使一些临界工作负载在内存耗尽前完成。性能成本范围从"零"到"显著"，具体取决于分配模式。默认值是无限的，即所有块都可以拆分。 `~torch.cuda.memory_stats`{.interpreted-text role="meth"} 和 `~torch.cuda.memory_summary`{.interpreted-text role="meth"} 方法对于调优很有用。此选项应作为最后手段，用于处理因"内存不足"而中止并显示大量非活动拆分块的工作负载。 `max_split_size_mb` 仅在 `backend:native` 时有效。对于 `backend:cudaMallocAsync`，`max_split_size_mb` 将被忽略。
...
\* `expandable_segments` (实验性功能，默认: [False]{.title-ref}) 如果设置为 [True]{.title-ref}，此设置会指示

:   分配器创建可以稍后扩展的 CUDA 分配，以更好地处理频繁改变分配大小的任务， 例如批量大小变化的情况。 通常对于大（\>2MB）分配，分配器会调用 cudaMalloc 来获取与用户请求大小相同的分配。 将来，如果这些分配是空闲的，其中的部分可以被其他请求重用。当程序进行许多 完全相同大小或大小为其整数倍的请求时，这种方法效果很好。许多深度学习模型遵循这种行为。 然而，一个常见的例外是批量大小在迭代之间略有变化，例如在批量推理中。 当程序最初以批量大小 [N]{.title-ref} 运行时，它将进行适合该大小的分配。 如果将来它以大小 [N - 1]{.title-ref} 运行，现有的分配仍然足够大。 但是，如果它以大小 [N + 1]{.title-ref} 运行，那么它将不得不进行稍大的新分配。 并非所有张量的大小都相同。有些可能是 [(N + 1)\*A]{.title-ref}，而其他可能是 [(N + 1)\*A\*B]{.title-ref}， 其中 [A]{.title-ref} 和 [B]{.title-ref} 是模型中的一些非批量维度。 因为分配器在现有分配足够大时会重用它们，所以一些 [(N + 1)\*A]{.title-ref} 分配实际上可以放入 已经存在的 [N\*B\*A]{.title-ref} 段中，尽管不是完美匹配。随着模型运行，它会部分填满所有这些段， 在这些段的末尾留下无法使用的空闲内存片。分配器在某个时刻将需要 [cudaMalloc]{.title-ref} 一个新的 [(N + 1)\*A\*B]{.title-ref} 段。如果没有足够的内存，现在就无法回收 现有段末尾的空闲内存片。对于 50+ 层的模型，这种模式可能会重复 50+ 次， 从而产生许多碎片。

    [expandable_segments]{.title-ref} 允许分配器最初创建一个段，然后在需要更多内存时扩展其大小。 它不是为每个分配创建一个段，而是尝试（每个流）创建一个可以根据需要增长的段。 现在，当运行 [N + 1]{.title-ref} 的情况时，分配将整齐地平铺到一个大段中，直到它被填满。 然后请求更多内存并附加到段的末尾。这个过程不会产生那么多无法使用的内存碎片， 因此更有可能成功找到所需内存。

\* [pinned_use_cuda_host_register]{.title-ref} 选项是一个布尔标志，用于确定是否
...
\* [pinned_num_register_threads]{.title-ref} 选项仅在 pinned_use_cuda_host_register

:   设置为 True 时有效。默认情况下，使用一个线程来映射页面。此选项允许 使用更多线程来并行化页面映射操作，以减少固定内存的总体分配时间。 根据基准测试结果，此选项的一个良好值是 8。

\* [pinned_use_background_threads]{.title-ref} 选项是一个布尔标志，用于启用后台线程

:   来处理事件。这避免了在快速分配路径中查询/处理事件相关的任何慢速路径。 此功能默认禁用。

\* [pinned_reserve_segment_size_mb]{.title-ref} 选项是以 MB 为单位的大小，用于为固定内存段保留。

:   这预先分配一大段固定内存，然后用于分配小尺寸请求。这有助于减少昂贵的设备库调用次数。

\* `graph_capture_record_stream_reuse` (实验性功能，默认: [False]{.title-ref})

:   如果设置为 [True]{.title-ref}，CUDA 缓存分配器将尝试在 CUDA 图捕获期间通过使用图拓扑（而不是 CUDA 事件） 来确定何时可以安全重用已释放的块，从而回收设备内存。这可以减少在长时间捕获期间 跨多个流释放和重新分配缓冲区的峰值内存使用，特别是当捕获 DAG 频繁到达连接前沿时。
...
`CUDA 内存管理 API<cuda-memory-management-api>`{.interpreted-text role="ref"} 报告的一些统计信息 是特定于 `backend:native` 的，对于 `backend:cudaMallocAsync` 没有意义。 详情请参阅每个函数的文档字符串。


## 为 CUDA 使用自定义内存分配器 {#cuda-memory-custom-allocator}
...
这可以通过 `torch.cuda.memory.CUDAPluggableAllocator`{.interpreted-text role="class"} 在 Python 中使用。 用户需负责提供 [.so]{.title-ref} 文件的路径以及符合上述签名的分配/释放函数名称。
...
根据您的使用场景，`~torch.cuda.change_current_allocator`{.interpreted-text role="meth"} 可能并非您所需，因为它会为整个程序交换 CUDA 分配器（类似于 `PYTORCH_ALLOC_CONF=backend:cudaMallocAsync`）。例如，如果交换的分配器没有缓存机制，您将失去 PyTorch 的 CUDACachingAllocator 的所有优势。相反，您可以使用 `torch.cuda.MemPool`{.interpreted-text role="class"} 有选择地标记 PyTorch 代码的某个区域以使用自定义分配器。这将允许您在同一 PyTorch 程序中使用多个 CUDA 系统分配器，同时保留 CUDACachingAllocator 的大部分优势（例如缓存）。通过使用 `torch.cuda.MemPool`{.interpreted-text role="class"}，您可以利用自定义分配器实现多种功能，例如：
...
以下代码展示了包装在 `torch.cuda.memory.CUDAPluggableAllocator`{.interpreted-text role="class"} 中的 `ncclMemAlloc`。
...
现在，您可以通过将此分配器传递给 `torch.cuda.MemPool`{.interpreted-text role="class"} 来定义一个新的内存池：

``` python
pool = torch.cuda.MemPool(allocator)
```

然后，该池可以与 `torch.cuda.use_mem_pool`{.interpreted-text role="class"} 上下文管理器一起使用，以将张量分配到该池中：
...
要回收内存，用户首先需要确保没有操作正在使用该内存池。当没有任何张量持有对该内存池的引用时，在内存池被删除时，`~torch.cuda.empty_cache`{.interpreted-text role="meth"} 会在内部被调用，从而将所有内存返还给系统。
...
以下 `torch.cuda.MemPool.use_count`{.interpreted-text role="meth"} 和 `torch.cuda.MemPool.snapshot`{.interpreted-text role="meth"} API 可用于调试目的：
...
- `torch.cuda.MemPool`{.interpreted-text role="class"} 持有对内存池的一个引用。当你使用 `torch.cuda.use_mem_pool`{.interpreted-text role="class"} 上下文管理器时，它也会获取对内存池的另一个引用。在退出上下文管理器时，它会释放其引用。此后，理想情况下应该只有张量持有对内存池的引用。一旦张量释放了它们的引用，内存池的使用计数将为 1，这反映出只有 `torch.cuda.MemPool`{.interpreted-text role="class"} 对象持有一个引用。只有在那时，当使用 `del` 调用内存池的析构函数时，内存池持有的内存才能被返还给系统。
- `torch.cuda.MemPool`{.interpreted-text role="class"} 目前不支持 CUDACachingAllocator 的 `expandable_segments` 模式。
...
## cuFFT 计划缓存 {#cufft-plan-cache}

对于每个 CUDA 设备，使用一个 cuFFT 计划的 LRU 缓存来加速在具有相同配置的相同几何形状的 CUDA 张量上重复运行 FFT 方法（例如 `torch.fft.fft`{.interpreted-text role="func"}）。因为一些 cuFFT 计划可能会分配 GPU 内存，所以这些缓存有最大容量。
...
要控制和查询非默认设备的计划缓存，您可以使用 `torch.device`{.interpreted-text role="class"} 对象或设备索引来索引 `torch.backends.cuda.cufft_plan_cache` 对象，并访问上述属性之一。例如，要设置设备 `1` 的缓存容量，可以写 `torch.backends.cuda.cufft_plan_cache[1].max_size = 10`。

## 即时编译 {#cuda-just-in-time-compilation}
...
第一步是确定是否应使用 GPU。一种常见模式是使用 Python 的 `argparse` 模块读取用户参数，并结合 `~torch.cuda.is_available`{.interpreted-text role="meth"} 设置一个可用于禁用 CUDA 的标志。在下文中，`args.device` 会生成一个 `torch.device`{.interpreted-text role="class"} 对象，可用于将张量移动到 CPU 或 CUDA。
...
在系统上使用多个 GPU 时，可以使用 `CUDA_VISIBLE_DEVICES` 环境标志来管理 PyTorch 可用的 GPU。如上所述，要手动控制张量创建在哪个 GPU 上，最佳实践是使用 `torch.cuda.device`{.interpreted-text role="any"} 上下文管理器。
...
如果您有一个张量，并希望在同一设备上创建相同类型的新张量，则可以使用 `torch.Tensor.new_*` 方法（参见 `torch.Tensor`{.interpreted-text role="class"}）。 虽然前面提到的 `torch.*` 工厂函数（`tensor-creation-ops`{.interpreted-text role="ref"}）依赖于当前 GPU 上下文和传入的属性参数，但 `torch.Tensor.new_*` 方法会保留张量的设备和其他属性。
...
如果您想创建与另一个张量相同类型和大小的张量，并用 1 或 0 填充，`~torch.ones_like`{.interpreted-text role="meth"} 或 `~torch.zeros_like`{.interpreted-text role="meth"} 提供了便捷的辅助函数（它们也保留张量的 `torch.device`{.interpreted-text role="class"} 和 `torch.dtype`{.interpreted-text role="class"}）。
...
当数据来自固定（页锁定）内存时，主机到 GPU 的复制会快得多。CPU 张量和存储暴露了一个 `~torch.Tensor.pin_memory`{.interpreted-text role="meth"} 方法，该方法返回对象的副本，并将数据放入固定区域。

此外，一旦固定了张量或存储，您就可以使用异步 GPU 复制。只需向 `~torch.Tensor.to`{.interpreted-text role="meth"} 或 `~torch.Tensor.cuda`{.interpreted-text role="meth"} 调用传递一个额外的 `non_blocking=True` 参数。这可以用于将数据传输与计算重叠。

您可以通过向 `~torch.utils.data.DataLoader`{.interpreted-text role="class"} 的构造函数传递 `pin_memory=True`，使其返回放置在固定内存中的批次。

### 使用 nn.parallel.DistributedDataParallel 替代 multiprocessing 或 nn.DataParallel {#cuda-nn-ddp-instead}

大多数涉及批量输入和多个 GPU 的用例应默认使用 `~torch.nn.parallel.DistributedDataParallel`{.interpreted-text role="class"} 来利用多个 GPU。

将 CUDA 模型与 `~torch.multiprocessing`{.interpreted-text role="mod"} 结合使用有重要的注意事项；除非精确满足数据处理要求，否则您的程序很可能出现不正确或未定义的行为。

建议使用 `~torch.nn.parallel.DistributedDataParallel`{.interpreted-text role="class"} 而不是 `~torch.nn.DataParallel`{.interpreted-text role="class"} 进行多 GPU 训练，即使只有一个节点。

`~torch.nn.parallel.DistributedDataParallel`{.interpreted-text role="class"} 和 `~torch.nn.DataParallel`{.interpreted-text role="class"} 的区别在于：`~torch.nn.parallel.DistributedDataParallel`{.interpreted-text role="class"} 使用多进程，为每个 GPU 创建一个进程，而 `~torch.nn.DataParallel`{.interpreted-text role="class"} 使用多线程。通过使用多进程，每个 GPU 都有其专用进程，这避免了 Python 解释器 GIL 带来的性能开销。

如果您使用 `~torch.nn.parallel.DistributedDataParallel`{.interpreted-text role="class"}，可以使用 [torch.distributed.launch]{.title-ref} 工具来启动您的程序，参见 `distributed-launch`{.interpreted-text role="ref"}。

## CUDA 图 {#cuda-graph-semantics}
...
如果您的网络全部或部分是图安全的（通常这意味着静态形状和静态控制流，但请参阅其他 `限制<capture-constraints>`{.interpreted-text role="ref"}），并且您怀疑其运行时至少在一定程度上受 CPU 限制，那么您应该尝试 CUDA 图。
...
PyTorch 通过原始的 `torch.cuda.CUDAGraph`{.interpreted-text role="class"} 类和两个便捷包装器 `torch.cuda.graph`{.interpreted-text role="class"} 和 `torch.cuda.make_graphed_callables`{.interpreted-text role="class"} 来暴露图。

`torch.cuda.graph`{.interpreted-text role="class"} 是一个简单、通用的上下文管理器，用于捕获其上下文中的 CUDA 工作。 在捕获之前，通过运行几次即时迭代来预热要捕获的工作负载。预热必须在侧流上进行。 因为图在每次重放时都从相同的内存地址读取和写入，所以您必须在捕获期间保持对保存输入和输出数据的张量的长期引用。 要在新的输入数据上运行图，请将新数据复制到捕获的输入张量，重放图，然后从捕获的输出张量读取新的输出。 示例:
...
有关实际和高级模式，请参阅 `全网络捕获<whole-network-capture>`{.interpreted-text role="ref"}、 `与 torch.cuda.amp 一起使用<graphs-with-amp>`{.interpreted-text role="ref"} 和 `与多流一起使用<multistream-capture>`{.interpreted-text role="ref"}。

`~torch.cuda.make_graphed_callables`{.interpreted-text role="class"} 更为复杂。 `~torch.cuda.make_graphed_callables`{.interpreted-text role="class"} 接受 Python 函数和 `torch.nn.Module`{.interpreted-text role="class"}s。对于每个传入的函数或模块， 它会分别创建前向传播和后向传播的计算图。请参阅 `部分网络捕获<partial-network-capture>`{.interpreted-text role="ref"}。

#### 约束条件 {#capture-constraints}

如果一组操作不违反以下任何约束，则它是\*可捕获的\*。

这些约束适用于 `torch.cuda.graph`{.interpreted-text role="class"} 上下文中的所有工作，以及您传递给 `torch.cuda.make_graphed_callables`{.interpreted-text role="func"} 的任何可调用对象的前向和后向传播中的所有工作。

违反以下任何一条都可能导致运行时错误：

- 捕获必须在非默认流上进行。（仅当您使用原始的 `CUDAGraph.capture_begin<torch.cuda.CUDAGraph.capture_begin>`{.interpreted-text role="meth"} 和 `CUDAGraph.capture_end<torch.cuda.CUDAGraph.capture_end>`{.interpreted-text role="meth"} 调用时才需关注此问题。 `~torch.cuda.graph`{.interpreted-text role="class"} 和 `~torch.cuda.make_graphed_callables`{.interpreted-text role="func"} 会为您设置一个侧流。）
- 禁止同步 CPU 和 GPU 的操作（例如，`.item()` 调用）。
- 允许 CUDA RNG 操作，并且在计算图中使用多个 `torch.Generator`{.interpreted-text role="class"} 实例时， 必须在图捕获之前使用 `CUDAGraph.register_generator_state<torch.cuda.CUDAGraph.register_generator_state>`{.interpreted-text role="meth"} 进行注册。 避免在捕获期间使用 `Generator.get_state<torch.get_state>`{.interpreted-text role="meth"} 和 `Generator.set_state<torch.set_state>`{.interpreted-text role="meth"}； 相反，应使用 `Generator.graphsafe_set_state<torch.Generator.graphsafe_set_state>`{.interpreted-text role="meth"} 和 `Generator.graphsafe_get_state<torch.Generator.graphsafe_get_state>`{.interpreted-text role="meth"} 来安全地管理计算图上下文中的生成器状态。这确保了 CUDA 计算图中正确的 RNG 操作和生成器管理。
- 禁止动态控制流（基于 CPU 或 GPU 数据），除非它基于 GPU 数据并通过高阶操作符 torch.cond() 实现。 请参阅 `数据依赖控制流<graph-data-dependent-control-flow>`{.interpreted-text role="ref"}。
...
- 允许在捕获中使用多个流，但存在 `限制<multistream-capture>`{.interpreted-text role="ref"}。

#### 非约束条件

- 一旦捕获，计算图可以在任何流上重放。

### 全网络捕获 {#whole-network-capture}
...
如果你的网络中有部分不适合捕获（例如，由于动态控制流、动态形状、CPU 同步或必要的 CPU 端逻辑），你可以以即时执行的方式运行这些不安全的部分，并使用 `torch.cuda.make_graphed_callables`{.interpreted-text role="func"} 仅对可安全捕获的部分进行图化。

默认情况下，`~torch.cuda.make_graphed_callables`{.interpreted-text role="func"} 返回的可调用对象是支持自动求导的，并且可以在训练循环中直接替换你传入的函数或 `nn.Module<torch.nn.Module>`{.interpreted-text role="class"}。

`~torch.cuda.make_graphed_callables`{.interpreted-text role="func"} 在内部创建 `~torch.cuda.CUDAGraph`{.interpreted-text role="class"} 对象，运行预热迭代，并根据需要维护静态输入和输出。因此（与 `torch.cuda.graph`{.interpreted-text role="class"} 不同），你不需要手动处理这些。

在以下示例中，数据相关的动态控制流意味着网络无法端到端捕获，但 `~torch.cuda.make_graphed_callables`{.interpreted-text role="func"} 允许我们无论如何都将图安全的部分捕获并作为图运行：:
...
### 与 torch.cuda.amp 一起使用 {#graphs-with-amp}

对于典型的优化器，`GradScaler.step<torch.cuda.amp.GradScaler.step>`{.interpreted-text role="meth"} 会使 CPU 与 GPU 同步，这在捕获期间是被禁止的。为了避免错误，要么使用 `部分网络捕获<partial-network-capture>`{.interpreted-text role="ref"}，要么（如果前向传播、损失计算和反向传播是捕获安全的）捕获前向传播、损失计算和反向传播，但不捕获优化器步骤：:
...
早于 2.9.6 版本的 NCCL 不允许捕获集合通信操作。 您必须使用 `部分网络捕获<partial-network-capture>`{.interpreted-text role="ref"}， 这将使 allreduce 操作延迟到反向传播的图外部分执行。

在使用 DDP 包装网络 *之前*，对可图化的网络部分调用 `~torch.cuda.make_graphed_callables`{.interpreted-text role="func"}。

#### NCCL \>= 2.9.6

2.9.6 或更高版本的 NCCL 允许在图中进行集合通信。 捕获 `整个反向传播过程<whole-network-capture>`{.interpreted-text role="ref"} 的方法是可行的选项，但需要三个设置步骤。
...
### 图内存管理 {#graph-memory-management}

捕获的图每次重放时都作用于相同的虚拟地址。 如果 PyTorch 释放了内存，后续重放可能会触发非法内存访问。 如果 PyTorch 将内存重新分配给新的张量，重放可能会破坏这些张量所看到的值。因此，图使用的虚拟地址必须在多次重放之间为图保留。PyTorch 缓存分配器通过检测捕获何时进行，并从图私有的内存池中满足捕获的分配来实现这一点。私有池会一直保持活动状态，直到其 `~torch.cuda.CUDAGraph`{.interpreted-text role="class"} 对象和捕获期间创建的所有张量都超出作用域。

私有池会自动维护。默认情况下，分配器为每次捕获创建一个独立的私有池。如果您捕获多个图，这种保守的方法确保图重放永远不会破坏彼此的值，但有时会不必要地浪费内存。

#### 跨捕获共享内存

为了节省私有池中占用的内存，`torch.cuda.graph`{.interpreted-text role="class"} 和 `torch.cuda.make_graphed_callables`{.interpreted-text role="func"} 可选地允许不同的捕获共享同一个私有池。 如果您知道一组图将始终按照它们被捕获的相同顺序重放，并且永远不会并发重放，那么它们共享一个私有池是安全的。

`torch.cuda.graph`{.interpreted-text role="class"} 的 `pool` 参数是使用特定私有池的提示，可用于在多个图之间共享内存，如下所示：:
...
在不相互依赖输出的独立图之间共享内存池也是安全的，前提是它们永远不会并发运行。 请注意，当它们共享一个池时，重放一个图可能会破坏另一个图的输出，除非事先对输出调用 `~torch.Tensor.clone`{.interpreted-text role="meth"}。 这种模式在运行时接受可变批次大小的推理服务器中经常使用。 vLLM 是一个显著的例子；参见 [此处](https://github.com/vllm-project/vllm/blob/938a81692ea318e59ead4750e7e7425bfd6a4896/vllm/platforms/interface.py#L508-L515) 和 [此处](https://github.com/vllm-project/vllm/blob/938a81692ea318e59ead4750e7e7425bfd6a4896/vllm/compilation/cuda_graph.py#L86-L89)。

对于 `torch.cuda.make_graphed_callables`{.interpreted-text role="func"}，如果您想要对多个可调用对象进行图化，并且知道它们将始终以相同的顺序运行（且永远不会并发），请按照它们在实时工作负载中运行的顺序将它们作为元组传递，`~torch.cuda.make_graphed_callables`{.interpreted-text role="func"} 将使用共享的私有池捕获它们的图。

如果在实时工作负载中，您的可调用对象的运行顺序偶尔会改变，或者它们将并发运行，则不允许将它们作为元组传递给单次 `~torch.cuda.make_graphed_callables`{.interpreted-text role="func"} 调用。相反，您必须为每个可调用对象分别调用 `~torch.cuda.make_graphed_callables`{.interpreted-text role="func"}。

### 数据依赖控制流 {#graph-data-dependent-control-flow}
