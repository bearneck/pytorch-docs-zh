...
## HIP 接口复用 CUDA 接口 {#hip_as_cuda}

用于 HIP 的 PyTorch 有意复用了现有的 `torch.cuda`{.interpreted-text role="mod"} 接口。这有助于加速现有 PyTorch 代码和模型的移植，因为几乎不需要（如果有的话）修改代码。

`cuda-semantics`{.interpreted-text role="ref"} 中的示例在 HIP 上完全适用：:
...
## 检查 HIP {#checking_for_hip}

无论你使用的是用于 CUDA 还是 HIP 的 PyTorch，调用 `~torch.cuda.is_available`{.interpreted-text role="meth"} 的结果都是相同的。如果你使用的是支持 GPU 的 PyTorch 构建版本，它将返回 [True]{.title-ref}。如果你必须检查正在使用的 PyTorch 版本，请参考以下示例：:
...
## ROCm 上的 TensorFloat-32(TF32) {#tf32_on_rocm}

ROCm 不支持 TF32。

## 内存管理 {#rocm-memory-management}

PyTorch 使用缓存内存分配器来加速内存分配。这允许在不进行设备同步的情况下快速释放内存。然而，分配器管理的未使用内存在 `rocm-smi` 中仍会显示为已使用。你可以使用 `~torch.cuda.memory_allocated`{.interpreted-text role="meth"} 和 `~torch.cuda.max_memory_allocated`{.interpreted-text role="meth"} 来监控张量占用的内存，并使用 `~torch.cuda.memory_reserved`{.interpreted-text role="meth"} 和 `~torch.cuda.max_memory_reserved`{.interpreted-text role="meth"} 来监控缓存分配器管理的总内存量。调用 `~torch.cuda.empty_cache`{.interpreted-text role="meth"} 会释放 PyTorch 中所有\*\*未使用的\*\*缓存内存，以便其他 GPU 应用程序可以使用这些内存。但是，张量占用的 GPU 内存不会被释放，因此无法增加 PyTorch 可用的 GPU 内存量。

对于更高级的用户，我们通过 `~torch.cuda.memory_stats`{.interpreted-text role="meth"} 提供更全面的内存基准测试。我们还提供通过 `~torch.cuda.memory_snapshot`{.interpreted-text role="meth"} 捕获内存分配器状态完整快照的能力，这可以帮助你理解代码产生的底层分配模式。
...
## hipFFT/rocFFT 计划缓存 {#hipfft-plan-cache}

不支持设置 hipFFT/rocFFT 计划缓存的大小。

## torch.distributed 后端 {#torch-distributed-backends}

目前，ROCm 上仅支持 torch.distributed 的 \"nccl\" 和 \"gloo\" 后端。

## C++ 中 CUDA API 到 HIP API 的映射 {#cuda-api-to_hip-api-mappings}
...
对于此处未列出的任何部分，请参考 CUDA 语义文档： `cuda-semantics`{.interpreted-text role="ref"}
