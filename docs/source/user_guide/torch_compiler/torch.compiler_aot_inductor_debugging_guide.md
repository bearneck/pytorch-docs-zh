...
- `AOTI_RUNTIME_CHECK_INPUTS=1` 检查输入是否满足编译期间使用的同一组守卫条件。更多详情请参阅 {ref}`torch.compiler_troubleshooting`。
...
- **tlparse / TORCH_TRACE**：提供完整的输出代码以供检查，并记录所使用的守卫条件集。更多详情请参阅 {ref}`tlparse / TORCH_TRACE <tlparse-torch-trace>`。
- **TORCH_LOGS**：使用 `TORCH_LOGS="+inductor,output_code"` 查看更多的 PT2 内部日志。更多详情请参阅 {ref}`TORCH_LOGS <torch-logs>`。
