...
这个 ``Tensor`` 类以及 ATen 中的所有其他符号都位于 ``at::`` 命名空间中，其文档见
`此处 <https://pytorch.org/cppdocs/api/namespace_at.html#namespace-at>`_。
...
ATen 中的 ``at::Tensor`` 类默认不可微分。要添加 autograd API 提供的张量可微性，你必须使用 `torch::` 命名空间中的张量工厂函数，而不是 `at::` 命名空间中的函数。例如，使用 `at::ones` 创建的张量将不可微分，而使用 `torch::ones` 创建的张量则是可微分的。
...
有关 C++ 前端的更详细描述，请参阅 `此文档 <https://pytorch.org/cppdocs/frontend.html>`_。与 C++ 前端相关的 `torch::` 命名空间的相关部分包括 `torch::nn
...
C++ 前端的示例可以在 `此代码库 <https://github.com/pytorch/examples/tree/master/cpp>`_ 中找到，该代码库正在持续积极地扩展。
...
PyTorch C++ 生态系统的此文档网站得益于 `Exhale <https://github.com/svenevs/exhale/>`_ 项目以及其维护者 `svenevs <https://github.com/svenevs/>`_ 慷慨投入的时间和精力。我们感谢 Stephen 的工作以及他为 PyTorch C++ 文档提供的帮助。